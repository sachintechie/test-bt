...
export const handler = async (event: any) => {
  try {
    ...
    const tokens = await getTokens(event.identity.resolverContext as tenant, event.arguments?.input?.walletAddress);
    return {
      status: 200,
      data: tokens,
      error: null
...
async function getTokens(tenant: tenant, walletAddress: string) {
