...
export class ProvenanceClient {
  ...
  async sendNft(sender: string, recipient: string, denomId: string, amount: string) {
    const client = await this.getSigningStargateClient();

    let SendNftTokenParams = {
      from_address: sender,
    ...
    let msgTypeURL = "/cosmos.bank.v1beta1.MsgSend";
    ...
    if (result.code !== 0) {
      throw new Error(`Failed to send NFT: ${result.code}`);
    }
