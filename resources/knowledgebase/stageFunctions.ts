...
async function processStage(
  ...
  try {
    ...
    const references = await getReferenceByProjectIdAndType(projectId, referenceStage, referenceStatus, RefType.DOCUMENT);
    ...
    for (const step of steps) {
      ...
      const stepInstance = await getStepByProjectId(tenantUserId, step.name, step.name, stepType.id, stage.id, steps.indexOf(step) + 1);
 ...
 */
export async function addStage_1(
  tenantId: string,
  tenantUserId: string,
  projectId: string,
  bucketName: string,
  ...
  try {
    ...
    if (project != null && project.data && project.data.knowledgebaseid == null) {
      ...
      if (project != null && kbResponse && kbResponse.data != null) {
        const updateProject = await updateProjectKbAndIndex(
          project.data.id,
          kbResponse.data.Kb_Id ?? "",
          kbResponse?.data.Index_Name ?? "",
          kbResponse?.data.Collection_Name ?? ""
        );
      ...
      [
        ...
        },
        {
          ...
          action: async (reference: any, stepId: string) => {
            ...
            const blockchainHashedData = await hashingAndStoreToBlockchain(reference.hash, chainType);
            console.log("metaData", blockchainHashedData);
            await createStepDetails(tenantUserId, JSON.stringify(blockchainHashedData.data), stepId, reference.id, ActionStatus.COMPLETED);
          }
        }
      ]
    );
...
export async function addStage_dataIngestion(
  tenantId: string,
  tenantUserId: string,
  projectId: string,
  bucketName: string,
    ...
    if (stageType5) {
      ...
      if (referenceList && referenceList.length > 0) {
        ...
        if (stage5 && stepType1 && project.data) {
          const step1 = await getStepByProjectId(
            tenantUserId,
            "Writing to open search",
            "Writing to open search",
            stepType1.id,
            stage5.id,
            1
