...
export class IndexS3Creation {
    ...
    async validateS3BucketName(bucketName: string): Promise<{ isValid: boolean, message: string }> {
        ...
        if(bucketName.includes('.-') || bucketName.includes('-.') || bucketName.startsWith('.') || bucketName.startsWith('-') || bucketName.endsWith('.') || bucketName.endsWith('-')) {
            return {
                isValid: false,
    ...
    async createKnowledgeBase(projectName: string): Promise<string> {
        ...
        this.bedrockAgentRuntimeClient.send(
