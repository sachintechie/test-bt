...
export const handler = async (event: any, context: any) => {
  try {
    ...
    const data = await addReference(resolverContext as tenant, refType, projectId, file, websiteName, websiteUrl, depth);
...
async function handleDocumentReference(file: any, dataStoredToDb: any) {
  ...
  if (s3PreHashedData.error) throw s3PreHashedData.error;
